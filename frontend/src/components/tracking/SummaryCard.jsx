...
const SummaryCard = ({ entry, timeline }) => {
  ...
  return (
    <div className="w-full mb-4">
      ...
      >
        <h3 className="text-xs text-red-100 font-black uppercase tracking-widest mb-2 flex items-center gap-1.5 px-5 relative z-30">
          ...
          {timeline && timeline.map((step, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`absolute bottom-full w-[160px] bg-white/95 backdrop-blur-md border border-white/40 p-3 rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 pointer-events-none shadow-2xl shadow-black/20 transform translate-y-3 group-hover:-translate-y-1 mb-3 flex flex-col items-center ${
                  ...
                  'left-[50%] -translate-x-1/2'
                   ...
                   {isCompleted ? (
                     <div className="w-full flex flex-col">
                       ...
                       {step.status?.toLowerCase() === 'received' && (
                         <div className="mt-2 w-full bg-amber-50 border border-amber-100/60 rounded-lg p-2 flex flex-col items-center shadow-sm">
                           <div className="text-xs font-black text-amber-700 uppercase tracking-wider flex items-center gap-1 mb-1">
                             <Clock size={10} /> Pending Action
                           </div>
                           <div className="text-xs text-amber-600/90 font-medium text-center leading-tight line-clamp-3 w-full">
                     ...
                     <div className="text-xs font-bold text-amber-600/80 mt-1 w-full text-center bg-amber-50 p-2 rounded-lg border border-amber-100/50">Awaiting arrival...</div>
                   )}
                ...
                <span className={`mt-2.5 text-xs font-black text-center leading-[1.3] transition-colors w-full break-words tracking-tight ${isCompleted ? 'text-white drop-shadow-sm' : 'text-red-200/50'}`}>
                  {step.department}
                </span>
